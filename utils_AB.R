...
# Comparaison des indicateurs de maniere simple et generique (differentes distances possibles)
compare.crps.simple <- function(k,nbdays,start="1950-01-01",end="2011-12-31",radtype="nrn05",standardize=TRUE,CV=TRUE,rean){
  ...
  # CRPS indicateurs
  crps.all <- crps.pos <- crps.p0 <- NULL
  lab <- NULL
  ...
  # CRPS moyen par indicateur
  meancrps.all <- apply(crps.all[idx,],2,mean)
  meancrps.pos <- apply(crps.pos[idx.pos,],2,mean)
  meancrps.p0 <- apply(crps.p0[idx,],2,mean)
  ...
  normalize.p0 <- mean(score$crps[idx,"p0 binom"])
  ...
  points(1:ndescr,1-meancrps.all/normalize.all,pch=19)
  lines(1:ndescr,1-meancrps.all/normalize.all,lty=3)
  ...
  points(1:ndescr,1-meancrps.extr/normalize.extr,pch=19)
  lines(1:ndescr,1-meancrps.extr/normalize.extr,lty=3)
...
compute_criteria<-function(k,dist,start="1950-01-01",end="2011-12-31",update=FALSE,rean,threeday=FALSE,rev=FALSE){
  gc()
  ...
  dist.vec<-getdist(k,ifelse(rev,"TWS",dist),start,end,rean,threeday) # si rev, on prend les nei en TWS
  gc()
...
compute_crps<-function(descriptors,k,dist,nbdays=3,start="1950-01-01",end="2011-12-31",standardize=TRUE,radtype="nrn05",CV=TRUE,rean,threeday=c(F,F)){
  ...
  print(paste0(path1,"compute_crps",get.CVstr(CV),"/",path2,".Rdata"))
  ...
  save(crps,file=paste0(path1,"compute_crps",get.CVstr(CV),"/",path2,".Rdata"))
...
# Desaisonnalise une serie de donnees
compute.desais <- function(descr,dates){
  
  sais <- aggregate(descr,by=list(substr(dates,6,10)),mean)
  pos <- match(substr(dates,6,10),sais[,1])
  sais.chro <- sais[pos,2]
  desais <- descr - sais.chro
}
...
fit.empir<-function(rean,k,descriptors,dist,nbdays=3,start="1950-01-01",end="2011-12-31",standardize=TRUE,radtype="nrn05",CV=TRUE,threeday=c(F,F)){
  ...
  for (i in 1:N){
    ...
    if (!is.na(descr[i,1]) & !is.na(descr[i,2])){
      ...
      ni<-length(pi)
      ni1<-sum(pi>0)
      count <- nn2(data = descr[tmp$idef,],query = t(descr[i,]),searchtype = "radius",radius =  0.5,k = length(tmp$idef)) # nombre de voisins dans le rayon (tmp$idef: indices ou les deux descr sont non NA)
...
getdata<-function(k,day0,day1=day0,rean=NULL,climat=NULL,run=1,large_win=F,small_win=F,all=F,ssp=NULL,pt.lon=NULL,pt.lat=NULL,lim.lon=NULL,lim.lat=NULL,return.lonlat=F,var="hgt"){
  ...
  tmp<-getinfo_window(k,large_win,small_win,all,rean,var,climat,run,ssp,pt.lon,pt.lat,lim.lon,lim.lat,return.lonlat)
  if(!return.lonlat){
    infowind <- tmp
  }else{ infowind <- tmp$info}
  ...
  if(!return.lonlat){
    ...
    return(list(data=data,lon=tmp$lon,lat=tmp$lat))
  }
}
...
getinfo_window<-function(k,large_win = F,small_win=F,all=F,rean=NULL,var="hgt",climat=NULL,run=1,ssp=NULL,pt.lon=NULL,pt.lat=NULL,lim.lon=NULL,lim.lat=NULL,return.lonlat=F){ 
  ...
  # Import
  nc <- load.nc(rean = rean,var = var,climat = climat,run = run,ssp = ssp)
  if(var == "hgt"){ nc <- nc[[k]]}
  ...
  }else if(!is.null(pt.lon)){# Point
    infolon <- c(which(lon==pt.lon),1)
    infolat <- c(which(lat==pt.lat),1)
  }else if(!is.null(lim.lon)){# Vecteur personnalise
    infolon <- c(which(lon==lim.lon[1]),which(lon==lim.lon[2])-which(lon==lim.lon[1])+1)
    infolat <- c(which(lat==lim.lat[1]),which(lat==lim.lat[2])-which(lat==lim.lat[1])+1)
  }else{# Incomplete
  ...
  }else{# si demande, on retourne directement les vecteurs lon et lat (pour cartes)
    lon <- lon[infolon[1]:(infolon[1]+infolon[2]-1)]
    lat <- lat[infolat[1]:(infolat[1]+infolat[2]-1)]
    return(list(info=rbind(infolon,infolat),lon=lon,lat=lat))
  }
}
...
# Calcul de la nouvelle acceleration sur trois jours
get.acc <- function(k,dist,start="1950-01-01",end="2011-12-31",rean){
  
  # Import de la celerite
  load(file=paste0(file=paste0(get.dirstr(k,rean),"compute_criteria/criteria_",dist,"_member",member,"_k",k,"_",start,"_",end,".Rdata")))
  cel <- criteria[,"cel"]
  
  # Traitement
  cel3 <- rollapply(cel,3,mean)
  accnew <- c(NA,cel3[-1]/cel3[-length(cel3)])
  accnew
}
...
get.ind.max <- function(type="year",nbdays=3,start="1950-01-01", end="2011-12-31",bv="Isere",spazm=F){
  ...
  # Si annee/mois incomplet, on ne calcule pas le max
  ele <- substr(dates,1,ifelse(type=="year",4,7)) # elements
  ele.length <- aggregate(dates,list(ele),length) # longueur de chaque element
  pos.NA <- which((ele.length[,2]) < (ifelse(type=="year",363,28))) # si pas anne/mois entier => NA a la fin de la fonction
...
# Max de precip issus de zonal ou meridional (Drac et Isere)
...
get.ind.max.sais <- function(sais="winter",wp=1,nbdays=3,start="1950-01-01", end="2011-12-31",bv="Isere",spazm=F){
  ...
  # Traitement
  pos.tmp <- apply(precip,2,which.max)
  pos <- pos.tmp
...
image.cumul<-function(crue=FALSE){
  ...
  if(!crue){
    ...
    debit <- c(crues[1,2],NA,NA,crues[2:38,2],NA,crues[39:40,2])
    ann <- 1969:2011
    tmp <- crues[,4]
    ...
    legend("bottomleft",inset=.02,pch=21,pt.bg="cornflowerblue",bty="n",
           pt.cex=c(max(debit/sd(debit,na.rm=T)/2,na.rm=T),median(debit/sd(debit,na.rm=T)/2,na.rm=T),min(debit/sd(debit,na.rm=T)/2,na.rm=T)),
...
load.nc<-function(rean = NULL,var="hgt",climat=NULL,run=1,ssp=NULL){
  ...
  if(!is.null(rean)){
    ...
    if(rean == "NCEP"){
      ...
      names(nc1000$var)[2] <- var
      nc1000$var[[2]]$name <- var
    }
    
    if(rean == "ERA5" & var == "hgt"){
      nc500<-nc_open("2_Travail/Data/Reanalysis/ERA5/HGT/ERA5_HGT500_1950-01-01_2021-07-17_daily.nc")
      nc1000<-nc_open("2_Travail/Data/Reanalysis/ERA5/HGT/ERA5_HGT1000_1950-01-01_2021-10-20_daily.nc")
      names(nc1000$var) <- var
      nc1000$var[[1]]$name <- var
    }
    
    if(rean == "ERA5_regrid_20CR"){
      nc500<-nc_open("2_Travail/Data/Reanalysis/ERA5/HGT/ERA5_HGT500_1950-01-01_2021-07-17_daily_regrid_20CR.nc")
      nc1000<-nc500
    }
    
    if(rean == "ERA5_regrid_ERA20C"){
      nc500<-nc_open("2_Travail/Data/Reanalysis/ERA5/HGT/ERA5_HGT500_1950-01-01_2021-07-17_daily_regrid_ERA20C.nc")
      nc1000<-nc500
    }
    ...
    if(rean == "ERA5" & substr(var,1,2) == "vv"){
      nc <-nc_open(paste0("2_Travail/Data/Reanalysis/ERA5/VV/ERA5_VV",substr(var,3,5),"_1950_2021_daily.nc"))
      names(nc$var) <- var
      nc$var[[1]]$name <- var
    }
    ...
    if(rean == "ERA5" & var == "topo"){
      nc <-nc_open("2_Travail/Data/Reanalysis/ERA5/TOPO/ERA5_TOPO.nc")
      names(nc$var) <- var
      nc$var[[1]]$name <- var
    }
...
plot.empir.clean.obs<-function(rean,k,descriptors,dist,nbdays=3,start="1950-01-01",end="2011-12-31",radtype="nrn05",CV=TRUE,threeday=c(F,F),dP=F,vari=F,maxi=F,noise=F,coin=F){
  ...
  # Creation du plot
  pdf(file=paste0(path1,"plot.empir.clean.obs",get.CVstr(CV),"/plot_",substr(path2,1,nchar(path2)-10),substr(path2,nchar(path2)-5,nchar(path2)),comp,"_obs.pdf"),width=8,height=5) # manip substr pour enlever le std
  par(mfrow=c(1,2))
  ...
  #if(coin){
  #  points(descr1[ale],descr2[ale],pch=21,cex=0.8,bg="white")
  #  points(descr1[ind.extr],descr2[ind.extr],pch=19,cex=0.8)}
  #title(names(param))
  ...
  if(vari) {
    ...
    var <- apply(geo,3,function(x) sd(x))
    var <- rollapply(var,nbdays,mean)
  ...
  if(maxi) {
    geo <- getdata(k = k[1],day0 = start,day1 = end,rean = rean[1]) 
    max <- apply(geo,3,function(x) max(x))
    max <- rollapply(max,nbdays,mean)
...
plot.sais.descr <- function(descr,k,nbdays=3,start="1950-01-01",end="2011-12-31",rean){
  ...
  min1 <- aggregate(descr1,by=list(substr(dates[-c(length(dates)-1,length(dates))],6,10)),min)
  min2 <- aggregate(descr2,by=list(substr(dates[-c(length(dates)-1,length(dates))],6,10)),min)
  
  max1 <- aggregate(descr1,by=list(substr(dates[-c(length(dates)-1,length(dates))],6,10)),max)
  max2 <- aggregate(descr2,by=list(substr(dates[-c(length(dates)-1,length(dates))],6,10)),max)
  ...
  plot(sais1[,2],type="n",axes=F,col="blue",lwd=2,xlab="Day",ylab=paste0(descr," TWS"),ylim=range(descr1))
  polygon(c(1:366,366:1),c(max1[,2],rev(min1[,2])),col="grey")
  lines(sais1[,2],col="blue",lwd=2)
  axis(2)
  axis(1,at = match(unique(substr(dates,6,7)),substr(dates,6,7)),labels = paste0("01/",unique(substr(dates,6,7))))
  box()
  
  plot(sais2[,2],type="n",axes=F,col="blue",lwd=2,xlab="Day",ylab=paste0(descr," RMSE"),ylim=range(descr2))
  polygon(c(1:366,366:1),c(max2[,2],rev(min2[,2])),col="grey")
  lines(sais2[,2],col="blue",lwd=2)
  axis(2)
  axis(1,at = match(unique(substr(dates,6,7)),substr(dates,6,7)),labels = paste0("01/",unique(substr(dates,6,7))))
  box()
...
plot.sais.dP.noise <- function(k,nbdays=3,start="1950-01-01",end="2011-12-31",rean){
  ...
  min2 <- aggregate(noise,by=list(substr(dates[-c(length(dates)-1,length(dates))],6,10)),min)
  max2 <- aggregate(noise,by=list(substr(dates[-c(length(dates)-1,length(dates))],6,10)),max)
  ...
  plot(sais1[,2],type="n",axes=F,col="blue",lwd=2,xlab="Day",ylab="Max pressure gradient (m)",ylim=range(dP))
  polygon(c(1:366,366:1),c(max1[,2],rev(min1[,2])),col="grey")
  lines(sais1[,2],col="blue",lwd=2)
  axis(2)
  axis(1,at = match(unique(substr(dates,6,7)),substr(dates,6,7)),labels = paste0("01/",unique(substr(dates,6,7))))
  ...
  plot(sais2[,2],type="n",axes=F,col="blue",lwd=2,xlab="Day",ylab="Noise (-)",ylim=range(noise))
  polygon(c(1:366,366:1),c(max2[,2],rev(min2[,2])),col="grey")
  lines(sais2[,2],col="blue",lwd=2)
  axis(2)
  axis(1,at = match(unique(substr(dates,6,7)),substr(dates,6,7)),labels = paste0("01/",unique(substr(dates,6,7))))
...
# Concatenation sous R de 1950-1978 (3 dims) et 1979-2021 (4 dims). Sortie 2x plus legere qu'avec cdo
reshape.ERA5.conc <- function(var="VV", z = "500"){
  ...
  if(var=="VV"){
    var.name <- "w"
    var.longname <- "Vertical Velocity"
    var.unit <- "Pa.s-1"
  }
  
  if(var=="SPH"){
    var.name <- "q"
    var.longname <- "Specific Humidity"
    var.unit <- "kg.kg-1"
  }
  ...
  arr.final <- abind(arr.1, arr.2, along = 3)
  dim(arr.final)
...
time.extr <- function(start="1950-01-01",end="2011-12-31"){
  ...
  barplot(sais,space=0,col="grey",ylab="Precipitation (mm)",xlab="Month",cex.lab=1.3,ylim=c(0,120))
  text(0.5:11.5,par("usr")[3]-1, labels = month.abb, srt = -45, pos = 1, xpd = TRUE)
  grid()
  ...
  hist(mon,breaks = 0:12,col="grey",border = "black",
